...
import {
  ...
  computeGPA, computeCGPA, gradeColor, gpaColor, uid,
} from "../lib/data";

const STORAGE_KEY = "academic_record_v2";
...
export default function Home() {
  ...
  useEffect(() => {
    if (hydrated) saveData(semesters);
  }, [semesters, hydrated]);
        ...
        <div className="shell header-inner">
          ...
          <div className="header-right">
            <div className="cgpa-block">
              <div className={`cgpa-num ${gpaColor(cgpa)}`}>{cgpa ? cgpa.toFixed(2) : "—"}</div>
      ...
      <main>
        <div className="shell">
              ...
              {semesters.map((sem) => {
                ...
                return (
                  <div key={sem.id} className="gpa-bar-wrap" onClick={() => setActiveSem(sem.id)}
                    style={{ cursor: "pointer" }} title={`${sem.label}: ${gpa ?? "—"}`}>
            ...
            <div className="sem-panel">
              ...
              <table className="subj-table">
                <thead>
                  <tr>
                    ...
                    <th className="td-name">Subject Name</th>
                  ...
                  {currentSem.subjects.map((subj) => (
                    <tr className="subj-row" key={subj.id}>
                      <td className="td-code">
                        <input
                          className="cell-input code-field"
                          value={subj.code ?? ""}
                          readOnly={!editMode}
                      ...
                      <td className="td-name">
                        <input
                          className="cell-input name-field"
                          value={subj.name}
                          readOnly={!editMode}
                      ...
                      <td className="td-cr">
                        <input
                          className="cell-input credit-field"
                      ...
                      <td className="td-grade">
                        {editMode ? (
                          <select
                            className={`grade-select ${gradeColor(subj.grade)}`}
                            value={subj.grade}
                      ...
                      {editMode && (
                        <td className="td-action">
                          <button className="btn-del-row" onClick={() => deleteSubject(currentSem.id, subj.id)} title="Remove subject">×</button>
                        </td>
                      )}
                    </tr>
                  ))}
              ...
              {editMode && (
                <div className="add-row-wrap">
                  <button className="btn-add-row" onClick={() => addSubject(currentSem.id)}>
                    + Add subject
                  </button>
                </div>
              )}
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div className="legend">
              {Object.entries(GRADE_POINTS).map(([g, p]) => (
                <span key={g} className={`legend-item grade-pill ${gradeColor(g)}`}
          ...
          <div className="bottom-bar">
            {editMode && (
              <button className="btn-add-sem" onClick={addSemester}>+ Add semester</button>
            )}
